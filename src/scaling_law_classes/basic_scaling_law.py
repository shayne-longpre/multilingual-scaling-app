...
class BasicScalingLaw(ScalingLaw):
    ...
    def torch_loss(inp: torch.Tensor, theta: torch.Tensor, *, tie: bool = True):
        ...
        return torch.nn.functional.huber_loss(post, torch.log(inp[:, 2]), delta=1e-3).sum()

    # --- NumPy pred for curve_fit --------------------------------------
    @staticmethod
    def numpy_loss(inp: np.ndarray, params: np.ndarray, *, tie: bool = True) -> np.ndarray:
        if tie:
    ...
    def N_to_D(self, N: float, target_loss: float, **other_vars) -> float:
        ...
        if denom <= 0:
            raise ValueError("No finite D can satisfy the loss at this N (denominator ≤ 0)")
    ...
    def DL_to_N(self, D, L):
        ...
        if L_eff <= 0:
            raise ValueError(f"Target loss {L} must exceed irreducible loss {p.irreducible}")
        
        denominator = L_eff - p.B / (D**p.beta)
        
        if denominator <= 0:
            raise ValueError(f"Cannot achieve loss {L} with {D} tokens - need more data")
