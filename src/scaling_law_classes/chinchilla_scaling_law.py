...
class ScalingLaw(ScalingLaw):
    ...
    def __init__(self, params: LawParams, form_str=None):#, variables=None):
        ...
        # params = symbols(self.params)
        # param_dict = {}
        # for i, param in enum(self.params.split(',')):
        #     param_dict[param.strip()] = self.params[i]
        # self.f = lambdify(params, parse_expr(form_str, transformations="all"), 'numpy')


    # Define the log-sum-exp function
    def form_exp_parts(self, inps: Dict):
        return [
            self.params['a'] - self.params['alpha'] * torch.log(inps['N']),
            self.params['b'] - self.params['beta'] * torch.log(inps['D']),
            self.params['e'].expand(inps['D'].shape[0])
        ]    
        ...
        for init_params in grid:
            if method == 'grid':
                ...
                if method == 'nonlinear_least_squares':
                    result = least_squares(obj, init_params, args=(inp_torch["N"][indices], inp_torch["D"][indices], inp_torch["Loss"][indices]), )
                else:
                    result = minimize(obj, init_params, args=(inp_torch["N"][indices], inp_torch["D"][indices], inp_torch["Loss"][indices]), tol=tol, method=method, jac=grad(obj) if use_grad else None)
