...
class ScalingLaw(ABC):
    ...
    def DL_to_N(self, D, L):
        """
        Minimum number of model params needed to reach L model loss after D training tokens.
        """
    ...
    def iso_compute_function(self, C: float, **other_vars):
        ...
        *analytic* solution; otherwise we fall back to a 1-D root-finder.
        ...
        def _N_to_D(N: float) -> float:
            fn = lambda D: self.flops(N=N, D=D, **other_vars) - C
            return brentq(fn, 1e-15, 1e15, maxiter=256)

        return _N_to_D


    def iso_loss_function(self, target_loss: float, **other_vars) -> Callable[[float], float]:
    ...
    def compute_optimal_allocation_inference(self, compute_budget, D_inference, **kwargs):
        """
        Compute optimal D, N given an expected performance (expressed via compute budget) and inference tokens

        See https://arxiv.org/abs/2401.00448 & https://github.com/nikhilsardana/beyond-chinchilla
        """
        if "unique_data" in kwargs or isinstance(self, DataConstrainedScalingLaw):
            ...
            raise NotImplementedError("Optimal inference allocation for data-constrained scaling laws not implemented yet.")
        
        # Allow also passing in target loss directly
        loss = kwargs.get('loss', self.compute_optimal_allocation(compute_budget)["loss"])
        ...
        approximation_error = abs(self.compute_optimal_train_tokens(D_opt, D_inference, loss))
        if approximation_error > 1e-6:
            raise RuntimeError(
                f"Could not find optimal training tokens. Error: {approximation_error}. "
                f"Try different initial guess or check if solution exists."
            )
