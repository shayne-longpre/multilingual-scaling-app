...
from src.scaling_law_classes.scaling_law import ScalingLaw, LawParams, ScalingLawWrapper
from src.helpers.plotting import (
    plot_optimal_scaling_line,
    plot_tokens_per_param_ratio,
)

st.set_page_config(
    page_title="Multilingual Scaling Laws",
    page_icon="📊",
    layout="wide"
)

st.title("Multilingual Scaling Laws Explorer")
st.markdown("An intuitive tool for understanding and interpreting scaling laws for machine learning models")

# Create tabs for the two main features
tab1, tab2 = st.tabs(["📊 Compute Budget Visualization", "📈 Fit Your Own Scaling Law"])
...
with tab1:
    ...
    with col1:
        ...
        input_mode = st.radio(
            "Choose input mode:",
            ["Compute Budget (FLOPs)", "Target Loss"]
        )
        ...
        with st.expander("Advanced Options"):
            inference_tokens = st.number_input(
                ...
                value=0,
                format="%d",
                help="Optional: Number of tokens for inference (0 = no inference optimization)"
            )
            ...
            total_tokens = st.number_input(
                ...
                value=0,
                format="%d",
                help="Optional: Total unique tokens in training set (0 = no data constraints)"
            )
        ...
        # Calculate button
        calculate_clicked = st.button("Calculate Optimal Allocation", type="primary", use_container_width=True)
    ...
    with col2:
        if calculate_clicked and selected_laws:
            ...
            for law_name in selected_laws:
                ...
                try:
                    if compute_budget is not None:
                        ...
                        st.warning(f"Target loss mode not yet implemented for {law_name}")
            ...
            if results:
                ...
                st.info("""
                **Key Insights:**
                - The scaling laws above are derived from specific papers and architectures
                - Consider whether your compute budget falls within the range tested in these papers
                - Architecture, data quality, and training setup can significantly affect the optimal allocation
                - For multilingual models, the scaling may differ from monolingual models
                ...
                **References:**
                ...
                for law_name in selected_laws:
                    law_wrapper = ALL_SCALING_LAWS[law_name]
                    st.markdown(f"- **{law_name}**: [{law_wrapper.paper}]({law_wrapper.paper})")

# Tab 2: Fit Your Own Scaling Law
with tab2:
    st.header("Fit Your Own Scaling Law")
    st.info("This feature is under development. Soon you'll be able to upload your own (N, D, Loss) data and fit custom scaling laws.")
    ...
    with col1:
        ...
        uploaded_file = st.file_uploader(
            ...
            type="csv",
            help="File should contain columns: N (model size), D (training tokens), Loss"
        )
            ...
            try:
                ...
                if law_type == "Data-Constrained Scaling Law":
                    if 'U' in df.columns:
                        ...
                        st.warning("Data-Constrained Scaling Law requires column 'U' (unique tokens)")
                
                # Fit button
                fit_clicked = st.button("Fit Scaling Law", type="primary", use_container_width=True)
    ...
    with col2:
        if uploaded_file is not None and fit_clicked:
            ...
            st.warning("Fitting functionality is not yet implemented. This will include:")
